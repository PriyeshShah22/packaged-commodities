...
async function imageRecord(file, index, automatic = false) {
  ...
  let thumbnail = '';
  if (image.width) { const canvas = document.createElement('canvas'); const scale = Math.min(1, 420 / image.width); canvas.width = Math.round(image.width * scale); canvas.height = Math.round(image.height * scale); canvas.getContext('2d').drawImage(image, 0, 0, canvas.width, canvas.height); thumbnail = canvas.toDataURL('image/jpeg', .65); }
...
export default function NewInspectionRoute() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      const report = { id, batchId: `BATCH-${products[0].key.split('-')[0]}`, date: new Date().toLocaleString('en-IN'), inspectorName: user?.name || '', status, overallReason, workflowStatus: 'OPEN', verificationScore, score: verificationScore, details: { productId: active.details.productId || active.declarations.barcode || id, productName: active.details.productName || active.declarations.commodity_name || 'Unnamed package', category: active.context.category, origin: active.context.origin, salesContext: active.context.salesContext }, declarations: active.declarations, fieldEvidence: active.ocrData?.fields || {}, counts: response.counts, results: response.results, violations: applicable.filter((item) => ['FAIL', 'REVIEW'].includes(item.outcome)), ocrLines, ocrStatus: ocrLines.length ? 'Text evidence detected' : 'No text evidence detected', imageCount: active.images.length, imageQuality: active.images.map(({ id: imageId, width, height, quality }) => ({ imageId, width, height, quality })), thumbnail: active.images[0]?.thumbnail || '' };
