...
export default function DashboardPlaceholder() {
  ...
  return <AppShell title="Compliance dashboard" eyebrow="OVERVIEW" actions={action}>
    ...
    <section className="bg-white border border-slate-200 rounded-2xl mt-5 overflow-hidden shadow-sm"><div className="p-5 border-b border-slate-200 flex justify-between items-center"><div><h3 className="font-extrabold">Recent reports</h3><p className="text-xs text-slate-500 mt-1">Product, evidence verification, and current assessment</p></div><button onClick={() => navigate('/reports')} className="text-sm font-bold text-sky-700 flex items-center gap-1">View all <ArrowRight className="w-4 h-4" /></button></div>{!reports.length ? <div className="p-12 text-center"><ClipboardCheck className="w-8 h-8 mx-auto text-slate-400" /><p className="font-bold mt-3">No product reports yet</p><p className="text-sm text-slate-500 mt-1">Run a scan to populate dashboard analytics.</p></div> : <div className="divide-y divide-slate-100">{reports.slice(0, 6).map((r) => <button key={r.id} onClick={() => navigate(`/reports/${r.id}`)} className="w-full px-5 py-4 flex items-center text-left hover:bg-slate-50"><div><p className="font-bold text-sm">{r.details?.productName || 'Unnamed product'}</p><p className="text-xs font-mono text-slate-500 mt-1">{r.id} · {r.date}</p></div><span className="ml-auto text-sm font-black">Evidence Verification: {r.verification?.verified ?? 0}/{r.verification?.total ?? 0} — {r.verificationScore ?? 0}%</span><span className={`ml-4 text-[10px] font-bold px-2 py-1 rounded-full ${r.status === 'COMPLIANT' ? 'bg-emerald-50 text-emerald-700' : r.status === 'NON_COMPLIANT' ? 'bg-rose-50 text-rose-700' : 'bg-amber-50 text-amber-700'}`}>{r.status === 'NON_COMPLIANT' ? 'POTENTIAL ISSUE' : r.status === 'REVIEW' ? 'NEEDS REVIEW' : 'PASS'}</span></button>)}</div>}</section>
