...
export default function DashboardPlaceholder() {
  ...
  return <AppShell title="Compliance dashboard" eyebrow="OVERVIEW" actions={action}>
    ...
    <section className="grid xl:grid-cols-[1.1fr_.9fr] gap-5 mt-5">
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm"><div className="flex items-center justify-between"><div><h3 className="font-extrabold">Compliance distribution</h3><p className="text-xs text-slate-500 mt-1">AI-assisted outcomes from submitted evidence</p></div><span className="text-3xl font-black text-sky-700">{metrics.rate}%</span></div><div className="mt-6 flex flex-col sm:flex-row items-center gap-8"><Donut {...metrics} /><div className="space-y-3 flex-1 w-full">{[['Compliant',metrics.compliant,'bg-emerald-500'],['Non-compliant',metrics.nonCompliant,'bg-rose-500'],['Review',metrics.review,'bg-amber-500']].map(([label,value,color]) => <div key={label} className="flex items-center"><span className={`w-2.5 h-2.5 rounded-full ${color}`} /><span className="ml-3 text-sm text-slate-600">{label}</span><span className="ml-auto font-bold">{value}</span></div>)}</div></div></div>
