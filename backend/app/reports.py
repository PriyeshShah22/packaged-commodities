...
def build_bulk_report_pdf(batch: dict[str, Any]) -> bytes:
    ...
    clear = sum(r.get("status") == "COMPLIANT" for r in reports); issues = sum(r.get("status") == "NON_COMPLIANT" for r in reports); review = sum(r.get("status") == "REVIEW" for r in reports)
    story = [Paragraph("BULK INSPECTION SUMMARY", styles["PMKicker"]), Paragraph("PackMetrix Multi-product Inspection", styles["PMTitle"]), Paragraph(f"<b>Batch / inspection ID:</b> {_safe(batch.get('id'))}<br/><b>Inspection date and time:</b> {_safe(batch.get('date'))}<br/><b>Inspector:</b> {_safe(batch.get('inspectorName'))}", styles["PMMeta"]), Spacer(1, 6 * mm)]
    counts = Table([[Paragraph("PRODUCTS ANALYZED", styles["PMLabel"]), Paragraph("CLEAR", styles["PMLabel"]), Paragraph("POTENTIAL ISSUES", styles["PMLabel"]), Paragraph("NEED REVIEW", styles["PMLabel"])], [Paragraph(str(len(reports)), styles["PMStatus"]), Paragraph(str(clear), styles["PMStatus"]), Paragraph(str(issues), styles["PMStatus"]), Paragraph(str(review), styles["PMStatus"])]], colWidths=[42.5 * mm] * 4)
    counts.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, -1), colors.HexColor("#F0F9FF")), ("BOX", (0, 0), (-1, -1), .6, colors.HexColor("#BAE6FD")), ("GRID", (0, 0), (-1, -1), .35, colors.HexColor("#BAE6FD")), ("ALIGN", (0, 0), (-1, -1), "CENTER"), ("PADDING", (0, 0), (-1, -1), 7)])); story += [counts, Spacer(1, 7 * mm), Paragraph("Product-wise summary", styles["PMSection"])]
    def main_finding(report):
        findings = [item for item in (report.get("results") or []) if item.get("outcome") in ("FAIL", "REVIEW")]
    ...
    rows = [["Product / GTIN", "Images", "Assessment", "Main finding", "Report ID"]] + [[_safe(f"{(r.get('details') or {}).get('productName') or 'Unidentified product'}\n{(r.get('declarations') or {}).get('barcode') or (r.get('details') or {}).get('productId') or 'GTIN not detected'}"), str(r.get("imageCount") or 0), _safe(_status(r)), _safe(main_finding(r)), _safe(r.get("id"))] for r in ordered]
    summary = Table([[Paragraph(f"<b>{c}</b>" if i == 0 else c, styles["PMSmall"]) for c in row] for i, row in enumerate(rows)], colWidths=[38 * mm, 17 * mm, 31 * mm, 49 * mm, 35 * mm], repeatRows=1)
    summary.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, 0), NAVY), ("TEXTCOLOR", (0, 0), (-1, 0), colors.white), ("GRID", (0, 0), (-1, -1), .35, BORDER), ("VALIGN", (0, 0), (-1, -1), "TOP"), ("PADDING", (0, 0), (-1, -1), 6)])); story += [summary, Spacer(1, 6 * mm), Paragraph("Each report ID references a separate product inspection record with its own evidence, declarations, findings, review state, and downloadable detailed PDF. Raw OCR and debug output are excluded from this bulk summary.", styles["PMSmall"])]
